        ...
        if(in_array($ext, $allowed)) {
            ...
            if(move_uploaded_file($_FILES['cv_file']['tmp_name'], $upload_dir . $new_filename)) {
				...
				if (mysqli_num_rows($check_query) != 0) {
					...
					// Using the updated user table instead of member table
					$query = "INSERT INTO user (name, email, password, mobile, university, college, cv_file, experiences, skills, education, other_info, type, is_approved) 
							VALUES ('$name', '$email', '$password', '$mobile', '$university', '$college', '$new_filename', '$experiences', '$skills', '$education', '$other_info', 'member', '$is_approved')";
...
<div class="form">
    <form method="post" role="form" class="php-email-form" enctype="multipart/form-data">
        ...
        <div class="text-center">
            <button type="submit" name="btn-submit">Add Member</button>
        </div>
    </form>
</div>
